...
const Navbar = () => {
  ...
  const dynamicWidth = isActive ? `93%` : `5%`; // dynamically changing the width of the search bar
      ...
      <nav>
        ...
        <div className="search-bar">
          ...
          <div onClick={() => setIsActive(!isActive)}>
            {isActive? <AiFillCloseCircle className="search"/>: <AiOutlineSearch className="search" />}
          </div>
        </div>
      </nav>
    </header>
